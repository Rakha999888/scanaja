    ...
    <div className="relative min-h-screen" style={{ background: '#f0ebff' }}>
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        ...
        <nav className="w-full sticky top-0 z-40 px-4 pt-4 pb-2">
          <div
            ref={mobileMenuRef}
            className="max-w-6xl mx-auto bg-white/90 backdrop-blur-md rounded-[1.75rem] border-2 border-white"
          ...
          >
            <div className="px-5 py-3 flex items-center justify-between">
              ...
              <button onClick={() => setMobileMenuOpen(v => !v)}
                ...
                <span className={`block w-5 h-0.5 bg-emerald-700 transition-all ${mobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`} />
                <span className={`block w-5 h-0.5 bg-emerald-700 transition-all ${mobileMenuOpen ? 'opacity-0' : ''}`} />
                <span className={`block w-5 h-0.5 bg-emerald-700 transition-all ${mobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
              </button>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-black text-gray-800">Dashboard Kehadiran</h1>
