...
export default function RegisterUser() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await setDoc(userRef, { email: form.email, password: form.password, createdAt: Date.now() })
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4 py-12" style={{ zIndex: 1 }}>
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <div className="mt-4">
              ...
              >
                ...
                <span className="text-xl font-black text-violet-700">Daftar Mahasiswa</span>
              </div>
            </div>
            <p className="text-sm font-semibold text-gray-400 mt-3">Buat akun baru untuk mulai absensi</p>
          </div>
          ...
          >
            ...
            <div className="mt-6 pt-5 border-t-2 border-gray-50 text-center">
              <p className="text-sm font-semibold text-gray-400">
                Sudah punya akun?{' '}
                <Link to="/login/user" className="font-extrabold text-violet-600 hover:text-violet-800 transition-colors">
                  Masuk di sini
                </Link>
              </p>
            </div>
          ...
          <div className="text-center mt-5">
            <Link to="/login/user" className="text-sm font-bold text-gray-400 hover:text-violet-500 transition-colors">
              ← Kembali ke login
            </Link>
          </div>
